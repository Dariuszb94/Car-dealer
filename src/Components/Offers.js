        ...
        <ul  className="offersContainer">
          {this.props.cars.cars==undefined ? null :  this.props.cars.cars.map((value, index) => {
            return <li key={index} className="offerContainer">     
                    <section className="photo"><img className="offerImage" src={require(`../Assets/${value.image1}.jpg`)}/></section>
                    <section className="offerText">
                      <article><div className="offerHeadName"><div><h2 className="offerHead offerHeadBrand ">{value.brand}</h2></div><div><h2 className="offerHead">{value.model}</h2></div></div></article>
                    ...
                    <section className="offerPrice">
                      <h2>{value.price}</h2>
                      <NavLink to="/offer" activeClassName="active"><button  value={value.id} className="seeOffer" onClick={this.submitId}>See offer</button></NavLink>
                    </section>
                  </li>
          })}
